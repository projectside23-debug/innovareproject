  ...
  return (
    <div className="bg-white">
      <section className="relative overflow-hidden bg-[#07141d] text-white">
        ...
        <div className="page-shell relative z-10 pb-14 pt-24 md:pb-28 md:pt-40">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[rgba(255,255,255,0.14)] bg-[rgba(255,255,255,0.08)] px-3 py-1.5 backdrop-blur-xl md:gap-3 md:px-4 md:py-2">
              <div className="relative h-7 w-7 overflow-hidden rounded-lg border border-[rgba(255,255,255,0.12)] md:h-8 md:w-8">
                <Image
                  alt="Innovare logo"
              ...
              <p className="text-[0.65rem] uppercase tracking-[0.2em] text-[rgba(255,255,255,0.68)] md:text-xs md:tracking-[0.24em]">
                Innovare / ecosystem platform
              </p>
            </div>

            <h1 className="display-title mt-4 text-[2.75rem] font-semibold leading-[0.92] text-white md:mt-6 md:text-[5.9rem] lg:text-[6.7rem]">
            ...
            <p className="mt-4 max-w-3xl text-sm leading-6 text-[rgba(255,255,255,0.74)] md:mt-6 md:text-lg md:leading-8">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-start gap-2 md:mt-8">
              <Link
                className="rounded-full bg-[#071426] px-5 py-2.5 text-xs font-semibold text-white shadow-[0_0_34px_rgba(140,201,219,0.26),0_18px_48px_rgba(4,12,18,0.24)] ring-1 ring-[#8cc9db]/25 transition hover:-translate-y-0.5 hover:bg-[#0b2031] md:px-7 md:py-3 md:text-sm"
                href="/industry"
              >
                Explore industry database
              </Link>
              <Link
                className="rounded-full border border-[rgba(255,255,255,0.16)] bg-[rgba(255,255,255,0.08)] px-5 py-2.5 text-xs text-white transition hover:bg-[rgba(255,255,255,0.12)] md:px-6 md:py-3 md:text-sm"
                href="/universities"
              >
            ...
            <div className="mt-6 flex flex-wrap gap-2 md:mt-10 md:gap-3">
              {ecosystemLabels.map((item, index) => (
                <span
                  className={`rounded-full border px-3 py-1.5 text-[0.65rem] uppercase tracking-[0.14em] backdrop-blur-sm md:px-4 md:py-2 md:text-xs md:tracking-[0.18em] ${index > 3 ? "hidden md:inline-flex" : "inline-flex"} ${item.className}`}
                  key={item.label}
                >
                  {item.label}
                </span>
      ...
      <section className="page-shell relative z-10 -mt-6 pb-6 md:-mt-14 md:pb-8">
        <div className="grid grid-cols-2 gap-3 rounded-[1.6rem] border border-[var(--line)] bg-white p-4 shadow-[0_26px_80px_rgba(8,20,30,0.08)] md:grid-cols-4 md:gap-4 md:rounded-[2rem] md:p-6">
          {trustedStats.map((stat) => (
            <article
              className={`rounded-[1.2rem] border border-[rgba(11,24,35,0.08)] p-4 shadow-[0_14px_42px_rgba(8,20,30,0.05)] md:rounded-[1.5rem] md:p-5 ${stat.cardClass}`}
              key={stat.label}
            >
              <p className={`text-3xl font-semibold tracking-[-0.06em] md:text-4xl ${stat.valueClass}`}>{stat.value}</p>
              <p className="mt-2 text-xs leading-5 text-[var(--ink-soft)] md:mt-3 md:text-sm md:leading-6">{stat.label}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="page-shell pb-6 md:pb-8">
        <article className="relative min-h-[10.5rem] overflow-hidden rounded-[1.7rem] border border-[var(--line)] shadow-[0_28px_90px_rgba(8,20,30,0.12)] md:min-h-[22rem] md:rounded-[2.2rem]">
          <Image
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 md:p-8">
            ...
            <p className="mt-2 max-w-2xl text-lg font-semibold leading-6 text-white md:mt-3 md:text-3xl md:leading-10">
      ...
      <section className="page-shell py-12 md:py-20" id="about">
        <SectionHeading
        ...
        <div className="mt-8 grid gap-4 md:gap-5 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="surface-card rounded-[1.6rem] p-4 md:rounded-[2rem] md:p-8">
            <p className="text-base leading-7 text-[var(--ink-soft)] md:text-lg md:leading-8">
            ...
            <div className="mt-5 grid gap-3 md:mt-8 md:gap-4">
              {quickCards.map((card) => (
                <div className={`rounded-[1.2rem] border border-[var(--line)] p-4 md:rounded-[1.5rem] md:p-5 ${card.className}`} key={card.title}>
                  <p className="text-sm font-semibold text-[var(--ink)]">{card.title}</p>
                  <p className="mt-2 text-sm leading-5 text-[var(--ink-soft)] md:leading-6">{card.copy}</p>
                </div>
              ))}
            </div>
          </div>

          <article className="relative min-h-[13rem] overflow-hidden rounded-[1.6rem] border border-[var(--line)] shadow-[0_30px_90px_rgba(8,20,30,0.12)] md:min-h-[28rem] md:rounded-[2rem]">
            <Image
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 md:p-8">
              ...
              <p className="mt-2 max-w-lg text-lg font-semibold leading-6 text-white md:mt-3 md:text-2xl md:leading-8">
      ...
      <section className="bg-[var(--background-strong)] py-12 md:py-20" id="why">
        <div className="page-shell">
          ...
          <div className="mt-8 grid gap-4 md:mt-10 md:gap-5 md:grid-cols-3">
            <div className="surface-card rounded-[1.5rem] p-4 md:rounded-[1.8rem] md:p-6">
              <p className="display-title text-xl font-semibold text-[#117ea0] md:text-2xl">For serious scanning</p>
              <p className="mt-3 text-sm leading-5 text-[var(--ink-soft)] md:mt-4 md:leading-6">
                Search, filter, and browse across categories without flattening different ecosystems into the same feed.
              </p>
            </div>
            <div className="rounded-[1.5rem] border border-[rgba(180,95,24,0.12)] bg-[linear-gradient(180deg,#fffaf2_0%,#ffffff_100%)] p-4 shadow-[0_28px_84px_rgba(180,95,24,0.08)] md:rounded-[1.8rem] md:p-6">
              <p className="display-title text-xl font-semibold text-[#b45f18] md:text-2xl">For early-career moves</p>
              <p className="mt-3 text-sm leading-5 text-[var(--ink-soft)] md:mt-4 md:leading-6">
                Use it to spot the kinds of firms, projects, and working opportunities that rarely show up in the right place.
              </p>
            </div>
            <div className="rounded-[1.5rem] border border-[rgba(20,116,95,0.12)] bg-[linear-gradient(180deg,#effff8_0%,#ffffff_100%)] p-4 shadow-[0_28px_84px_rgba(20,116,95,0.08)] md:rounded-[1.8rem] md:p-6">
              <p className="display-title text-xl font-semibold text-[#14745f] md:text-2xl">For venture-minded people</p>
              <p className="mt-3 text-sm leading-5 text-[var(--ink-soft)] md:mt-4 md:leading-6">
      ...
      <section className="page-shell py-12 md:py-20" id="industry">
        <SectionHeading
        ...
        <div className="mt-8 grid gap-4 md:mt-10 md:gap-5 lg:grid-cols-2">
          {exploreCards.map((card) => (
            <article
              className={`overflow-hidden rounded-[1.6rem] border border-[var(--line)] p-4 shadow-[0_24px_80px_rgba(10,20,32,0.08)] md:rounded-[2rem] md:p-8 ${card.className}`}
              key={card.title}
            >
              <p className="eyebrow text-xs text-[var(--ink-soft)]">{card.eyebrow}</p>
              <h3 className="display-title mt-3 max-w-xl text-3xl font-semibold text-[var(--ink)] md:mt-4 md:text-4xl">
                {card.title}
              </h3>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--ink-soft)] md:mt-4 md:leading-7">{card.copy}</p>
              <Link className="cta-dark mt-5 inline-flex rounded-full px-4 py-2.5 text-xs transition md:mt-8 md:px-5 md:py-3 md:text-sm" href={card.href}>
                {card.cta}
              </Link>
            </article>
          ))}
